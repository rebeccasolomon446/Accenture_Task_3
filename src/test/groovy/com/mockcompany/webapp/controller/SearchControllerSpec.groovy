...
class SearchControllerSpec extends Specification {
    ...
    def "search will return an item that matches the name of the product"() {
        // def keyword used to declare a dynamically typed variable in Groovy, followed by the test case
        given: // keyword used in Spock (Groovy testing framework) to declare preconditions of a test case
        // two ProductItem objects are created using the createItem() method on line 26
        ...
        when: // declares the action that is being tested.
        Set<ProductItem> results = doSearch("Will") // method created on line 22 being called with keyword "Will"

        then: // used to declare the expected outcomes of the test case
        results.contains(item1) // expect search results to contain item1
